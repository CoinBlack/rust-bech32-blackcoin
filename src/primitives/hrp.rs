...
// Defines HRP constants for the different blackcoin networks.
// You can also access these at `crate::hrp::BLK` etc.
#[rustfmt::skip]
...
define_hrp_const! {
    /// The human-readable part used by the Blackcoin mainnet network.
    pub const BLK 3 [98, 108, 107, 0];
}
define_hrp_const! {
    /// The human-readable part used by the Blackcoin testnet networks (testnet, signet).
    pub const TBLK 4 [116, 98, 108, 107];
}
define_hrp_const! {
    /// The human-readable part used when running a Blackcoin regtest network.
    pub const BLRT 4 [98, 108, 114, 116];
}
...
impl Hrp {
    ...
    ///
    /// This does _not_ check that the `hrp` is an in-use HRP within Blackcoin (eg, "blk"), rather it
...
mod tests {
    ...
    #[test]
    fn hrp_consts() {
        use crate::primitives::hrp::{BLK, BLRT, TBLK};
        assert_eq!(BLK, Hrp::parse_unchecked("blk"));
        assert_eq!(TBLK, Hrp::parse_unchecked("tblk"));
        assert_eq!(BLRT, Hrp::parse_unchecked("blrt"));
    }
